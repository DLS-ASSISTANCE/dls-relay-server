...
wss.on('connection', ws => {
    ...
    ws.on('message', data => {
        ...
        if(msg.type==='connect-to-host') {
            type='client';
            id='c_'+Date.now();
            const h=hosts.get(msg.hostId);
            if(!h){ws.send(JSON.stringify({type:'host-not-found'}));return;}
            if(h.password&&h.password!==msg.password){ws.send(JSON.stringify({type:'auth-failed'}));return;}
            clients.set(id,{ws,hostId:msg.hostId});
            ws.send(JSON.stringify({type:'auth-success'}));
            h.ws.send(JSON.stringify({type:'client-connected'}));
        }
        if(msg.type==='screen-data'&&type==='host') {
            clients.forEach(c=>{
                if(c.hostId===id&&c.ws.readyState===1) c.ws.send(data);
            });
        }
        if(['mouse-click','mouse-dblclick','mouse-rightclick','key-press','scroll'].includes(msg.type)&&type==='client') {
            const c=clients.get(id);
            if(c){const h=hosts.get(c.hostId);if(h)h.ws.send(data);}
        }
    });
    ws.on('close', () => {
        browsers.delete(ws);
        if(type==='host'){hosts.delete(id);broadcastHostsList();}
